...
class EflCrawler(AsyncCrawler):
    ...
    async def start(self):
        _, page_content = await self.extract_async(self.site_url)

        soup = BeautifulSoup(page_content, 'html.parser')
        for x in soup.find_all("div", {"class": "hidden"}):
            for a in x.find_all("a"):
                if "/AuctionList/" in a.get("href"):
                    category_uri = a.get("href")
                    category = category_uri.replace("/AuctionList/", "").replace("/", "-")
                    log.debug(self.site_url + category_uri)
                    _, page = await self.extract_async(self.site_url + category_uri)
                    cat_id, max_page_no = self.get_pagination(page)

                    log.debug("Cat: %s(%d) max page: %d" % (category, cat_id, max_page_no))
    ...
    async def get_pagination(self, page_content):
        _, page_content = await self.extract_async(self.site_url)
        soup = BeautifulSoup(page_content, 'html.parser')
        for x in soup.find_all("div", {"class": "hidden"}):
            for a in x.find_all("a"):
                if "/AuctionList/" in a.get("href"):
                    ...
                    log.debug("Cat: %s(%d) page: %d" % (category, cat_id, max_page_no))

        soup = BeautifulSoup(page_content, 'html.parser')
        pagination = soup.find("div", {"class": "pager"})
        if pagination is not None:
            page_li_selected = pagination.find("li", {"class", "selected"})
            if page_li_selected is not None:
                a = page_li_selected.find("a")
                if a is not None:
                    page_uri = a.get("href")
                    search = re.search("category=([0-9]+)", page_uri, re.IGNORECASE)
                    if search is not None:
                        log.debug("URL: %s, cat id=%d" % (self.site_url + page_uri, int(search.group(1))))
                        cat_id = int(search.group(1))

                        max_page = 0
                        for page_goto in pagination.find_all("li"):
                            page_a = page_goto.find("a")
                            if page_a is not None:
                                search_page_no = re.search("page=([0-9]+)", page_a.get("href"), re.IGNORECASE)
                                if search_page_no is not None:
                                    max_page = max(max_page, int(search_page_no.group(1)))
                        return cat_id, max_page
...
async def main():
    async with EflCrawler() as efl_crawl:
        await efl_crawl.start()
